...
public class MapaPlanDao{	
	...
	public boolean insertReg( MapaPlan plan ) {
		...
		try{
			String comando = "INSERT INTO ENOC.MAPA_PLAN" 
				...
				+ " ESTADO, ENLINEA, CICLOS, NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN)"
				+ " VALUES( ?, ?, ?,"
				+ " TO_DATE(?,'DD/MM/YYYY'),"
				+ " TO_DATE(?,'DD/MM/YYYY'),"
				+ " TO_DATE(?,'DD/MM/YYYY'),"
			...
			Object[] parametros = new Object[] {
					...
					plan.getPrecio(), plan.getRvoeInicial(), plan.getVersionId(), plan.getSemsys(), plan.getExpediente(), plan.getCreditos(), plan.getPlanOrigen()};
	...
	public boolean updateReg( MapaPlan plan ) {
		...
		try{
			...
			Object[] parametros = new Object[] {
					...
					plan.getSemsys(), plan.getExpediente(), plan.getCreditos(), plan.getPlanOrigen(), plan.getPlanId()}; 				
			if (salomonJdbc.update(comando,parametros)==1){
				ok = true;
			}			
		}catch(Exception ex){
		...
		try{
			...
			if(salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN,"
						...
						+ " NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
						+ " FROM ENOC.MAPA_PLAN"
						+ " WHERE PLAN_ID = ? ";					
				plan = salomonJdbc.queryForObject(comando, new MapaPlanMapper(), parametros);
			}			
		}catch(Exception ex){
	...
	public String getNombrePlanUtil( String planId) {
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				...
				nombrePlan = salomonJdbc.queryForObject(comando, String.class, parametros);
			}						
		}catch(Exception ex){
	...
	public int getNumCursos( String planId) {
		...
		try{
			...
			Object[] parametros = new Object[] {planId};			
			nCursos = salomonJdbc.queryForObject(comando, Integer.class, parametros);						
		}catch(Exception ex){
	...
	public String getCarreraSe( String planId) {		
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1){
				comando = "SELECT CARRERA_SE FROM ENOC.MAPA_PLAN WHERE PLAN_ID = ?";
				nombreCarrera = salomonJdbc.queryForObject(comando, String.class, parametros);
			}			
		}catch(Exception ex){
	...
	public String getPlanDeOrigen( String planId) {
		...
		try {
			String comando = "SELECT COUNT(*) FROM ENOC.MAPA_PLAN WHERE PLAN_ID = ?";	
			if (salomonJdbc.queryForObject(comando, Integer.class, planId) >= 1){		
				comando = "SELECT PLAN_ORIGEN FROM ENOC.MAPA_PLAN WHERE PLAN_ID = ?";			
				origen = salomonJdbc.queryForObject(comando, String.class, planId);
			}		
		}catch(Exception ex){
	...
	public String getNumPlanes( String carreraId, String estado) {
		...
		try{
			...
			carrera = salomonJdbc.queryForObject(comando, String.class,carreraId,estado);			
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|getNumPlanes|:"+ex);
		}
		return carrera;
	...
	public String getNumPlanesAdmision( String carreraId) {
		...
		try{
			...
			carrera = salomonJdbc.queryForObject(comando, String.class, carreraId);			
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|getNumPlanes|:"+ex);
		}		
		return carrera;
	...
	public int getNumPlanesPorEstado( String estado) {
		...
		try{
			...
			total = salomonJdbc.queryForObject(comando, Integer.class, estado);			
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|getNumPlanes|:"+ex);
		}		
		return total;
	...
	public String getNivelId( String planId) {
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				...
				nivel = salomonJdbc.queryForObject(comando, String.class, parametros);
			}					
		}catch(Exception ex){
	...
	public String getCarreraIdPLAN(String planId) {
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				...
				carreraId = salomonJdbc.queryForObject(comando, String.class, parametros);
			}						
		}catch(Exception ex){
	...
	public String getNotaExtraPlan( String planId){
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				...
				notaExtraPlan = salomonJdbc.queryForObject(comando, String.class, parametros);
			}			
		}catch(Exception ex){
	...
	public String getPrecio(String planId ){		
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				...
				precio = salomonJdbc.queryForObject(comando, String.class, parametros);	
			}
		}catch(Exception ex){
	...
	public String getRvoe( String planId) {
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				...
				revoe = salomonJdbc.queryForObject(comando, String.class, parametros);	
			}							
		}catch(Exception ex){
	...
	public String getOficial( String planId) {
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class, parametros) >= 1) {
				...
				oficial = salomonJdbc.queryForObject(comando, String.class, parametros);
			}						
		}catch(Exception ex){
	...
	public List<MapaPlan> getListAll( String orden ) {
		...
		try{
			comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO,"
					...
					+ " RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
					+ " FROM ENOC.MAPA_PLAN "+ orden;
			
			lista = salomonJdbc.query(comando, new MapaPlanMapper());			
		}catch(Exception ex){
	...
	public List<MapaPlan> listPlanesEnCarga(String cargaId, String orden ) {
		...
		try{
			String comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO, TO_CHAR(F_FINAL,'dd/mm/yyyy') AS F_FINAL,"
					...
					+ " NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN FROM ENOC.MAPA_PLAN"
			...
			lista = salomonJdbc.query(comando, new MapaPlanMapper(), parametros);			
		}catch(Exception ex){
	...
	public List<MapaPlan> getLista( String carreraId, String orden ){
		...
		try{
			comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, F_INICIO, F_FINAL,"
					...
					+ " NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
					+ " FROM ENOC.MAPA_PLAN"
					+ " WHERE CARRERA_ID = ?"
					+ " AND ESTADO IN ('A', 'V') "+ orden;
			
			lista = salomonJdbc.query(comando, new MapaPlanMapper(),carreraId);
	...
	public List<MapaPlan> getListPlanFacAll( String facultad, String orden ) {
		...
		try{
			comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO, TO_CHAR(F_FINAL,'dd/mm/yyyy')F_FINAL,"
					...
					+ " ENLINEA, CICLOS, NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
					+ " FROM ENOC.MAPA_PLAN"
					+ " WHERE ENOC.FACULTAD(CARRERA_ID) = ? "
					+ " AND ESTADO IN ('A', 'V','I') "+ orden;
			
			lista = salomonJdbc.query(comando, new MapaPlanMapper(),facultad); ///
	...
	public List<MapaPlan> listAutorizadosEnLinea( String modalidadId, String orden ){
		List<MapaPlan> lista	= new ArrayList<MapaPlan>();	
		try{
			String comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO, TO_CHAR(F_FINAL,'dd/mm/yyyy') F_FINAL,"
					+ " TO_CHAR(F_ACTUALIZA,'dd/mm/yyyy') F_ACTUALIZA, NUM_CURSOS, MINIMO, MAXIMO, CARRERA_SE, RVOE, OFICIAL, ESTADO,"
					+ " ENLINEA, CICLOS, NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
					+ " FROM ENOC.MAPA_PLAN"
					+ " WHERE ENLINEA IN ('S','M')"
					+ " AND CARRERA_ID IN (SELECT CARRERA_ID FROM SALOMON.ADM_MODCARRERA WHERE MODALIDAD_ID = to_number(?,'99')) "+ orden;
			lista = salomonJdbc.query(comando, new MapaPlanMapper(), modalidadId);			
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|listAutorizadosEnLinea|:"+ex);
		}
		return lista;
	}
	
	public List<MapaPlan> listAutorizadosEnLinea( String modalidadId, String estados, String orden ){
		List<MapaPlan> lista	= new ArrayList<MapaPlan>();	
		try{
			String comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO, TO_CHAR(F_FINAL,'dd/mm/yyyy') F_FINAL,"
					+ " TO_CHAR(F_ACTUALIZA,'dd/mm/yyyy') F_ACTUALIZA, NUM_CURSOS, MINIMO, MAXIMO, CARRERA_SE, RVOE, OFICIAL, ESTADO,"
	...
	public List<MapaPlan> listPlanesPorPeriodoAndModo(String periodoId, String enLinea, String orden ){
		List<MapaPlan> lista = new ArrayList<MapaPlan>();		
		try{
			String comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO, TO_CHAR(F_FINAL,'dd/mm/yyyy')F_FINAL,"
					+ " TO_CHAR(F_ACTUALIZA,'dd/mm/yyyy') F_ACTUALIZA, NUM_CURSOS, MINIMO, MAXIMO, CARRERA_SE, RVOE, OFICIAL, ESTADO,"
					+ " ENLINEA, CICLOS, NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
					+ " FROM ENOC.MAPA_PLAN"
					+ " WHERE PLAN_ID IN (SELECT PLAN_ID FROM SALOMON.ADM_INGRESO_PLAN WHERE PERIODO_ID = ? AND MODALIDAD_ID IN (SELECT MODALIDAD_ID FROM ENOC.CAT_MODALIDAD WHERE ENLINEA = ?))"+ orden;			
			lista = salomonJdbc.query(comando, new MapaPlanMapper(), periodoId, enLinea);			
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|listPlanesPorPeriodoAndModo|:"+ex);
		}
		return lista;
	}

	public List<MapaPlan> listPlanesPorPeriodoAndModoAndNivel(String periodoId, String enLinea, String nivelEstudio, String orden ){
		List<MapaPlan> lista = new ArrayList<MapaPlan>();		
		try{
			Object[] parametros = new Object[] {periodoId, enLinea, nivelEstudio};
			String comando = "SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO, TO_CHAR(F_FINAL,'dd/mm/yyyy')F_FINAL,"
					+ " TO_CHAR(F_ACTUALIZA,'dd/mm/yyyy') F_ACTUALIZA, NUM_CURSOS, MINIMO, MAXIMO, CARRERA_SE, RVOE, OFICIAL, ESTADO,"
					+ " ENLINEA, CICLOS, NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO, RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
					+ " FROM ENOC.MAPA_PLAN"
					+ " WHERE PLAN_ID IN (SELECT PLAN_ID FROM SALOMON.ADM_INGRESO_PLAN WHERE PERIODO_ID = ? AND MODALIDAD_ID IN (SELECT MODALIDAD_ID FROM ENOC.CAT_MODALIDAD WHERE ENLINEA = ?)) AND PLAN_SE = ?"+ orden;			
			lista = salomonJdbc.query(comando, new MapaPlanMapper(), parametros);			
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|listPlanesPorPeriodoAndModoAndNivel|:"+ex);
		}
		return lista;
	}
	...
	public HashMap<String, String> mapRvoeAlum( String cargaId, String orden) {
		...
		try{
			...
			lista = salomonJdbc.query(comando, new adm.MapaMapper(),cargaId);
	...
	public List<MapaPlan> lisPlanesEnCarga( String cargaId ) {
		...
		try{
			...
			lista = salomonJdbc.query(comando, new MapaPlanMapper(), cargaId);			
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|lisPlanesEnCarga|:"+ex);
		}
		return lista;
	...
	public boolean cotejado( String planId ) {
		...
		try{
			...
			if (salomonJdbc.queryForObject(comando, Integer.class,planId) >= 1){
				...
				cotejado = salomonJdbc.queryForObject(comando, String.class,planId);
			}				
		}catch(Exception ex){
			System.out.println("Error - aca.plan.spring.MapaPlanDao|cotejado|:"+ex);
		}
	...
	public void cambiaCotejado( String planId,String cotejado) {
		...
		try{
			...
			salomonJdbc.update(comando,parametros);
		}catch(Exception ex){
	...
	public List<MapaPlan> getListPorPlan(String planId, String orden) {
		List<MapaPlan> lista		= new ArrayList<MapaPlan>();
		...
		try{
			String comando = " SELECT PLAN_ID, CARRERA_ID, NOMBRE_PLAN, TO_CHAR(F_INICIO,'dd/mm/yyyy') AS F_INICIO,"
					...
					+ " NOTA_EXTRA, GENERAL, PLAN_SE, NOMBRE_PLAN_MUJER, CLAVE_PROFESIONES, PRECIO,"
					+ " RVOE_INICIAL, VERSION_ID, CLAVE_SEMSYS, EXPEDIENTE, CREDITOS, PLAN_ORIGEN"
					+ " FROM ENOC.MAPA_PLAN WHERE PLAN_ID = ? "+ orden; 
