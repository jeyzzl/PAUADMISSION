...
public class AdmPagoDao{
	...
	public AdmPago mapeaRegId(String folio) {
		...
		try {
			...
			if (salomonJdbc.queryForObject(comando,Integer.class,parametros)>=1){
				...
				objeto = salomonJdbc.queryForObject(comando, new AdmPagoMapper(),parametros);
			}	
		}catch(Exception ex){
	...
	public List<AdmPago> listPagos (String folio){
		...
		try {
			String comando = "SELECT FOLIO, CANTIDAD, TO_CHAR(FECHA,'YYYY-MM-DD') AS FECHA, METODO, COMENTARIO, RECIBO, ARCHIVO, NOMBRE"
				+ " FROM SALOMON.ADM_PAGO WHERE FOLIO = TO_NUMBER(?,'9999999')";
			Object[] parametros = new Object[] {folio};
			lista = salomonJdbc.query(comando, new AdmPagoMapper(), parametros);
		}catch(Exception ex) {
