...
<head>
	...
	<script type="text/javascript">
		function confirmar(){
			if(confirm("If you're ready to submit your application, click 'Accept'")){
...
<%
	...
	AdmPago admPago 			= (AdmPago)request.getAttribute("admPago");
	...
	List<AdmDocAlum> lisDocsAlum = (List<AdmDocAlum>)request.getAttribute("lisDocsAlum");
	
	HashMap<String,CatPais> mapaPaises = (HashMap<String,CatPais>)request.getAttribute("mapaPaises");
	HashMap<String,AdmAcomodo> mapaAcomodo = (HashMap<String,AdmAcomodo>)request.getAttribute("mapaAcomodo");
	...
	if(mapaAcomodo.containsKey(admSolicitud.getAcomodoId())){
		nombreAcomodo = mapaAcomodo.get(admSolicitud.getAcomodoId()).getNombreAcomodo();
	}
%>
<body>
				...
				<div class="d-flex bd-highlight page-header">	
					...
					<div class="p-2 flex-shrink-1 bd-highlight">
						<div class="d-flex align-items-center">
				    		<a href="<%=request.getContextPath()%>/referencias?Folio=<%=folio%>"><i class="fas fa-caret-left fa-3x"></i></a>
				    		&nbsp;<b class="fs-5">12/12</b>&nbsp;
    						...
    						<div id="personal" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
      							<div class="accordion-body">
        							<dl class="form-group">
	        							...
	        							<dd><%=admSolicitud.getNombre()%> <%=admSolicitud.getApellidoPaterno()%> <%=admSolicitud.getApellidoMaterno()==null ? "" : admSolicitud.getApellidoMaterno()%></dd>
	        							<dt><spring:message code="solicitud.confirmar.LugarNacimiento"/>:</dt>
	        							<dd>
	        								<%=!admSolicitud.getPaisId().equals(admParametros.getPaisId()) || ciudad==null ? "" : ciudad+"," %>
											<%=!admSolicitud.getPaisId().equals(admParametros.getPaisId()) || estadoId==null ? "" : estadoId+","%>
	 										<%=paisId==null ? "" : paisId%>
	 									</dd>
	        							...
	        							<dd><%=admSolicitud.getFechaNac() == null ? "" : admSolicitud.getFechaNac()%></dd>
...
<%			  							String estadoCivil = "-";
							            if(admSolicitud.getEstadoCivil()!= null){
							              	if(admSolicitud.getEstadoCivil().equals("C")){
							                	if(admSolicitud.getGenero().equals("M")){
							                		estadoCivil="Married";
							                	}else {
							                		estadoCivil="Married";
							                	}
							              	}else if(admSolicitud.getEstadoCivil().equals("S")){
								                if(admSolicitud.getGenero().equals("M")){
								                	estadoCivil="Single / Never Married";
								                }else{
								                	estadoCivil="Single / Never Married";
								                }
						             		}else if(admSolicitud.getEstadoCivil().equals("V")){
								                if(admSolicitud.getGenero().equals("M")){
								                	estadoCivil="Widowed";
								                }else{
								                	estadoCivil="Widowed";
								                }
							             	}else if(admSolicitud.getEstadoCivil().equals("D")){
								                if(admSolicitud.getGenero().equals("M")){
  						...
  						<div class="accordion-item">	 <!-- DATOS PADRES-->
  						...
  						<div class="accordion-item">  <!-- DATOS ACADEMICOS-->
   	 						...
   	 						<div id="academica" class="accordion-collapse collapse" aria-labelledby="headingTwo" data-bs-parent="#accordionExample">
      							<div class="accordion-body">
        							<dl class="form-group">
										...
										<dt>Study Level</dt>
										<dd>
											<%=admSolicitud.getNivelEstudio().equals("U")?"Undergraduate":""%>
											<%=admSolicitud.getNivelEstudio().equals("P")?"Postgraduate":""%>
										</dd>
									<% if(admSolicitud.getNivelEstudio().equals("U")){%>
										<dt>Application Type</dt>
										<dd>
											<%=admSolicitud.getTipo().equals("S")?"School Leaver":""%>
											<%=admSolicitud.getTipo().equals("N")?"Non-School Leaver":""%>
										</dd>
									<%	}%>
										<dt>Nationality Type</dt>
										<dd>
											<%=admSolicitud.getTipoAplicante().equals("N")?"National":""%>
											<%=admSolicitud.getTipoAplicante().equals("I")?"International":""%>
										</dd>
						        	</dl>
      							</div>
    						</div>
  						</div>
  						<div class="accordion-item">  <!-- DATOS BACKGROUND STUDIES-->
...
<%  								}%>
						...
						<div class="accordion-item">  		<!-- DATOS ACOMODO-->
    						<h2 class="accordion-header" id="acomodoTitle">
      							<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#acomodo" aria-expanded="false" aria-controls="acomodo">
       								<spring:message code="solicitud.confirmar.InfoAcomodo"/>
      							</button>
    						</h2>
							<div id="acomodo" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
								<div class="accordion-body">
									<dl class="form-group">
										<dt><spring:message code="solicitud.confirmar.AcomodoTitulo" /></dt>
										<dd><%=nombreAcomodo%></dd>
									</dl>
								</div>
							</div>
						</div>
  						<div class="accordion-item"> 	 <!-- DATOS SALUD-->
  						...
  						<div class="accordion-item">		 <!-- DATOS GUARDIAN-->
...
<% 									}%>
  					  	...
  					  	<div class="accordion-item">		 <!-- DATOS RECOMENDACIONES-->
...
<%									}%>
 						...
 						<div class="accordion-item">		 <!-- DATOS BANCO-->
						    <h2 class="accordion-header" id="bancoTitle">
						      	<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#banco" aria-expanded="false" aria-controls="banco">
						        	<spring:message code="banco.titulo"/>
						      	</button>
						    </h2>
						    <div id="banco" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
						      	<div class="accordion-body">
									<dl class="form-group">
						          		<dt>Bank</dt>
						          		<dd><%=admBanco.getBanco()%></dd>
					          			<dt>Account Number</dt>
						          		<dd><%=admBanco.getCuentaNumero()%></dd>
										<dt>Account Name</dt>
						          		<dd><%=admBanco.getCuentaNombre()%></dd>
										<dt>BBS Number</dt>
						          		<dd><%=admBanco.getNumeroBbs()%></dd>
										<dt>Account Type</dt>
						          		<dd><%=admBanco.getCuentaTipo()%></dd>
									</dl>
						      	</div>
						    </div>
					  	</div>
 						<div class="accordion-item">		 <!-- DATOS DOCUMENTOS-->
						    <h2 class="accordion-header" id="documentosTitle">
						      	<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#documentos" aria-expanded="false" aria-controls="documentos">
						        	<spring:message code="documentos.documentos.RequisitosAdmision"/>
						      	</button>
						    </h2>
						    <div id="documentos" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
						      	<div class="accordion-body">
									<dl class="form-group">
						<%	for(AdmDocAlum documento : lisDocsAlum){
								AdmDocumento doc = mapDocumentos.get(documento.getDocumentoId());	
						%>
						          		<dt><%=doc.getDocumentoNombre()%></dt>
						          		<dd><%=documento.getEstado().equals("E")?"Sent":""%></dd>
						<%	}%>
									</dl>
						      	</div>
						    </div>
					  	</div>
 						<div class="accordion-item">		 <!-- DATOS PAGO-->
						    <h2 class="accordion-header" id="referenciasTitle">
						      	<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#referencias" aria-expanded="false" aria-controls="referencias">
						        	Application Fee
						      	</button>
						    </h2>
						    <div id="referencias" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
						      	<div class="accordion-body">
									<dl class="form-group">
						          		<dt>Receipt Number</dt>
						          		<dd><%=admPago.getRecibo()%></dd>
					          			<dt>Reference</dt>
						          		<dd><%=admPago.getComentario()%></dd>
										<dt>Ammount</dt>
						          		<dd><%=admPago.getCantidad()%></dd>
									</dl>
						      	</div>
						    </div>
					  	</div>
<%--  						<div class="accordion-item">		 <!-- REGLAMENTO-->
						    ...
						    <div id="reglamento" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
						      	<div class="accordion-body">
					        		<a href="<%=request.getContextPath()%>/StudentHandbook.pdf" target="_blank"><spring:message code="solicitud.confirmar.AbrirReglamento"/></a>
						      	</div>
						    </div>
					  	</div> --%>
					</div>
...
<%	if(admParametros.getInstitucion().equals("Pacific Adventist University")){%>
		      				<spring:message code="solicitud.confirmar.MensajeCompromiso.Pau"/>
<%  }%>
<%	if(admParametros.getInstitucion().equals("Sonoma")){%>
		      				<spring:message code="solicitud.confirmar.MensajeCompromiso.Sonoma"/>
<%  }%>	
<%	if(admParametros.getInstitucion().equals("Fulton")){%>
		      				<spring:message code="solicitud.confirmar.MensajeCompromiso.Fulton"/>
<%  }%>		
			   			</p><br>
<%	if(admSolicitud.getEstado().equals("3")){ %>	
...
<% 	}else{%>
					...
					<div class="text-center">	
						...
						<button type="button" class="btn alert-info" onclick="document.location.href='../inicio';">
				 			OK <span class="fas fa-check fa-2x" aria-hidden="true"></span>
						</button>
					</div>
<%	}%>
				</form>
