...
public class Solicitud {
	...
	public String maximoReg(Connection conn) throws SQLException{
		...
		try{
			ps = conn.prepareStatement("SELECT NVL(MAX(FOLIO), 0)+1 AS MAXIMO FROM SALOMON.ADM_SOLICITUD"); 
			rs = ps.executeQuery();
			if (rs.next())
	...
	public String getEdad(Connection conn) throws SQLException{
		...
		try{
			ps = conn.prepareStatement("SELECT FLOOR(MONTHS_BETWEEN(SYSDATE, PER_NACIMIENTO)/12) AS EDAD" +
					" FROM SALOMON.ADM_SOLICITUD" + 
					" WHERE FOLIO = TO_NUMBER(?, '99999999')");
