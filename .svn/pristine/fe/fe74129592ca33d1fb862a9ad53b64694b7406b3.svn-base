...
<%
	...
	boolean tieneSolicitudActiva 		= (boolean)request.getAttribute("tieneSolicitudActiva");
%>
<body>
<div class="container-fluid p-0">
	...
	<%	if (existeUsuario){%>
		<hr><a class="btn btn-primary text-decoration-none <%=tieneSolicitudActiva?"disabled":""%>" href="crearNuevaSolicitud?UsuarioId=<%=admUsuario.getUsuarioId()%>" role="button"><spring:message code="inicio.crearNuevaSolicitud"/></a>
	<%	} %>
	</div>
	<div class="row m-4">
		<%
		...
		for(AdmSolicitud solicitud : lisSolicitudes){
			...
			// GET ESTADO
			if(solicitud.getEstado().equals("1")) estado = "Account Generated";
			if(solicitud.getEstado().equals("2")) estado = "Completed Form";
			if(solicitud.getEstado().equals("3")) estado = "Completed Documents";
			if(solicitud.getEstado().equals("4")) estado = "Submitted";
			if(solicitud.getEstado().equals("5")) estado = "Accepted";
			...
			if(solicitud.getEstado().equals("1")) buttonTxt = "Start Application <i class='fas fa-play ms-1'></i>";
			if(solicitud.getEstado().equals("2") || solicitud.getEstado().equals("3")) buttonTxt = buttonTxt = "Continue Application <i class='fas fa-play ms-1'></i>";
			if(solicitud.getEstado().equals("4") || solicitud.getEstado().equals("5")) buttonTxt = "View Application <i class='fas fa-eye ms-1'></i>";
%>
		<div class="card border-dark w-25 p-0 me-4" style="min-width: 23rem;">
			...
			<div class="card-body">
				...
				<ul class="list-group mb-4">
					<li class="list-group-item"><b><spring:message code="inicio.fechaSolicitud"/>:</b> <%=fecha%></li>
					<li class="list-group-item"><b><spring:message code="inicio.estadoSolicitud"/>:</b>  <span class='badge rounded-pill text-dark bg-warning'><%=estado%></span></li>
					<li class="list-group-item"><b><spring:message code="adm.Carrera"/>:</b> <span class='badge rounded-pill text-dark bg-warning'><%=carrera%></span></li>
				</ul>
				<a href="<%=request.getContextPath()%>/inicial?Folio=<%=folio%>" class="btn btn-primary text-decoration-none me-3"><%=buttonTxt%></a>
<%		String botonBorrar = " ";
				if(mapAcademico.containsKey(solicitud.getFolio())) {
...
We will review it and contact you with the next steps. Please check the status on your dashboard for updates.
				</div>
<%		}%>
			</div>
		</div>
