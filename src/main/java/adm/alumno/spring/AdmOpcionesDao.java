...
public class AdmOpcionesDao {
	...
	public AdmOpciones mapeaRegId(String opcionId){
		...
		try {
			Object[] parametros = new Object[] {opcionId};
				...
				objeto = salomonJdbc.queryForObject(comando, new AdmOpcionesMapper(),parametros);				
		}catch(Exception ex){
	...
	public String maxReg() {
		...
		try{
			...
			max = salomonJdbc.queryForObject(comando,String.class);
		}catch(Exception ex){
	...
	public List<AdmOpciones> getAll(String orden) {
		...
		try{
			...
			lista = salomonJdbc.query(comando, new AdmOpcionesMapper());			
		}catch(Exception ex){
