...
public class AdmPasosDao {
	...
	public AdmPasos mapeaRegId(String folio, String paso) {
		...
		try {
			String comando = "SELECT FOLIO, TO_CHAR(FECHA, 'DD/MM/YYYY') AS FECHA,"
					...
					+ " AND PASOS = ?";
			Object[] parametros = new Object[] { folio, paso };
			admPasos = salomonJdbc.queryForObject(comando, new AdmPasosMapper(), parametros);	
		} catch (Exception ex) {
	...
	public List<AdmPasos> getAll( String orden) {
		...
		try{
			...
			lista = salomonJdbc.query(comando, new AdmPasosMapper());
		}catch(Exception ex){
	...
	public HashMap<String, AdmPasos> mapaPasos(){
		...
		try {
			String comando = "SELECT FOLIO, TO_CHAR(FECHA, 'YYYY/MM/DD HH24:MI:SS') AS FECHA, USUARIO, PASOS, ESTADO, COMENTARIO"
					+ " FROM SALOMON.ADM_PASOS";
			lista = salomonJdbc.query(comando, new AdmPasosMapper());
